...
## 한 줄 소개
CareerFlow AI는 이력서/포트폴리오/채용공고를 입력받아 **공고 대상 장·단점 분석 -> 보완 -> 문서 생성 -> 면접 준비**까지 이어지는 저장형 취업 지원 워크플로우 MVP입니다.
...
- **핵심 로직**:
  - 갭 분석(`matched/missing/weak`)을 바탕으로 강점·약점·추천 보완 문장을 결정적으로 조합(별도 수치 점수 없음)
- **코드 포인트**:
  - `backend/src/common/fit-analysis.util.ts`
  - `frontend/components/results-client.tsx` (`FitAnalysisPanel`)

### 2) 대화형 보완
- **목적**: 부족 근거를 짧은 입력으로 보완하고 프로필·갭·장·단점 스냅샷을 재계산
- **핵심 로직**:

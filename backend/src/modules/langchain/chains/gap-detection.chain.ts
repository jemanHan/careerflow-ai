...
export async function runGapDetectionChain(
  ...
  const prompt = PromptTemplate.fromTemplate(
    [
      ...
      "- matchedSignals: JD asks for it AND candidate JSON or project text has clear supporting evidence (role, deliverable, outcome, tool, metric, deployment, collaboration, or artifact). Use a short evidence-style phrase in Korean, not bare tech tokens alone.",
      "- missingSignals: JD clearly requires it AND neither candidate JSON nor any project/summary field contains a meaningful related mention. If the same theme appears multiple times across summary/experience/projects, do NOT treat it as missing.",
      "- weakEvidence: Related topic appears in candidate data BUT proof is thin, indirect, or needs quantification (preferred-only gaps, niche patterns like RAG/Agent without depth).",
      "",
      "Hard rules:",
      "- Before filling missingSignals, scan candidate.strengths, experiences.impact, projects.description/evidence, and summary for keywords that satisfy the requirement.",
      "- matchedSignals must NOT be only isolated keywords like 'typescript' unless paired with context in the same field.",
